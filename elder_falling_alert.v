...
module top_module (
    ...
    input miso,             // SPI MISO from ADXL345
    ...
    // Threshold for detecting fall (tuned as needed)
    parameter THRESH_FREEFALL = 16'd10;
    parameter THREE_SECONDS = 300_000_000;  // 3 seconds @ 100 MHz

   
    wire [4:0] z_data_short = acl_data[4:0];  // 5-bit Z data from spi_master
    wire signed [15:0] z_data_ext = { {11{z_data_short[4]}}, z_data_short }; // sign-extend to 16-bit

    reg [31:0] fall_counter = 0;

    // Instantiate SPI Master
    spi_master accel_reader (
        .iclk(clk),        // Assuming clk is 4MHz (adjust if not)
        .miso(miso),
        .sclk(sclk),
        .mosi(mosi),
        .cs(cs),
    ...
    always @(posedge clk or posedge rst) begin
        if (rst) begin
            ...
            if (z_data_ext < THRESH_FREEFALL) begin
                if (fall_counter < THREE_SECONDS)
